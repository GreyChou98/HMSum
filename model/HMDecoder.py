...
from model.neural import PositionwiseFeedForward
from model.transformer_encoder import PositionalEncoding
...
class TransformerDecoderLayer(nn.Module):
    ...
    def __init__(self, d_model, heads, d_ff, dropout, padding_idx):
        ...
        self.context_attn = MultiHeadedAttention(
            heads, d_model, dropout=dropout)
        self.cluster_attn = HierClusterAttention(d_model)
    ...
    def forward(self, inputs, src, src_features, cluster_feature, cluster_mask, edge,
        ...
        selected_features, selected_mask = self.cluster_attn(cluster_feature, cluster_mask,
...
class TransformerDecoder(nn.Module):
    def __init__(self, num_layers, d_model, heads, d_ff, dropout, embeddings):
        ...
        self.linear_input = nn.Linear(d_model, d_model)
        self.linear_cluster = nn.Linear(d_model, d_model)
        self.softmax_cluster = nn.Softmax(dim=-1)
        self.transformer_layers = nn.ModuleList(
            [TransformerDecoderLayer(d_model, heads, d_ff, dropout, self.embeddings.padding_idx)
    ...
    def forward(self, tgt, memory_bank, state, edge, cluster, cluster_feature, memory_lengths=None,
                step=None, cache=None,memory_masks=None):
        """
        :param tgt: tgt_len x batch_size
        :param memory_bank:  batch_size x n_paras x n_tokens x embed_dim
        :param state:
        :param edge: batch_size x n_paras x n_clusters
        :param cluster: batch_size x n_clusters x n_tokens
        :param cluster_feature: batch_size x n_clusters x hidden
        ...
        :return:
        """
        batch_size, n_paras, n_cluster = edge.size()
        src = state.src  # n_blocks * n_tokens, batch_size
        src_words = src.transpose(0, 1).contiguous()
        src_words = src_words.view(batch_size, n_paras, -1)  # batch_size x n_blocks x n_tokens
        ...
        cluster_mask = ~(cluster.data.eq(padding_idx).bool())
        cluster_mask = (torch.sum(cluster_mask, -1) > 0).bool()
        ...
        for i in range(self.num_layers):
            output, all_input \
                = self.transformer_layers[i](output, src_words, memory_bank, cluster_feature, cluster_mask, edge,
...
# class HierDecoderLayerCluster(nn.Module):
#     def __init__(self, d_model, heads, d_ff, dropout):
#         super(HierDecoderLayerCluster, self).__init__()
#
#

class HierClusterAttention(nn.Module):
    def __init__(self, model_dim, dropout=0.1):
        self.model_dim = model_dim

        super(HierClusterAttention, self).__init__()
        self.linear_input = nn.Linear(model_dim, model_dim)
        self.linear_cluster = nn.Linear(model_dim, model_dim)
        self.softmax = nn.Softmax(dim=-1)
        self.dropout = nn.Dropout(dropout)

    def forward(self, cluster, cluster_mask, edge, inputs, src, src_features, padding_idx, max_para=5):
        """
        :param cluster: batch_size x n_clusters x hidden
        :param cluster_mask: index the pad cluster nodes: batch_size x n_clusters, 1 indicates the None nodes
        :param edge: batch_size x n_paras x n_clusters
        ...
        :return:
        """
        key = self.linear_cluster(cluster)
        query = self.linear_input(inputs)

        scores = torch.matmul(query, key.transpose(1, 2))       # batch_size x tgt_len x n_clusters

        if cluster_mask is not None:
            cluster_mask  = cluster_mask.unsqueeze(1).expand_as(scores).bool()
            scores = scores.masked_fill(cluster_mask,-1e18)

        attn_cluster = self.softmax(scores)     # batch_size x tgt_len x n_clusters
        attn_cluster = self.dropout(attn_cluster)
        attn_para = torch.matmul(attn_cluster, edge.transpose(1, 2))        # batch_size x tgt_len x n_paras
        ...
        for i in range(attn_para.size(0)):
            ...
            word.append(_word)
        selected_features = torch.stack(feature, dim=0)  # batch_size x tgt_len x max_paras x n_tokens x embed
        selected_words = torch.stack(word, dim=0)  # batch_size x tgt_len x max_paras x n_tokens
