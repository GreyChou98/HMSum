...
from model.beam import GNMTGlobalScorer
from model.cal_rouge import test_rouge, rouge_results_to_str
from model.neural import tile
...
class Translator(object):
    ...
    def _fast_translate_batch(self,
        ...
        edge = batch.edge
        para_state, src_features, cluster_state = self.model.encoder(src, cluster, edge)
        ...
        edge = tile(edge, beam_size, dim=0)
        cluster = tile(cluster, beam_size, dim=0)
        cluster_state = tile(cluster_state, beam_size, dim=0)
        ...
        for step in range(max_length):
            ...
            #                                              step=step)
            dec_out, dec_states = self.model.decoder(decoder_input, src_features, dec_states,
                                                     edge, cluster, cluster_state, step=step)
            ...
            edge = edge.index_select(0, select_indices)
            cluster = cluster.index_select(0, select_indices)
            cluster_state = cluster_state.index_select(0, select_indices)
