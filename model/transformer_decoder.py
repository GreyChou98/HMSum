...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, memory_bank, state, memory_lengths=None,
        ...
        src_words = src.transpose(0, 1)    # batch_size, n_blocks * n_tokens
        tgt_words = tgt.transpose(0, 1)    # batch_size, max_len
        ...
        src_memory_bank = memory_bank.transpose(0, 1).contiguous()    # real_batch_size, max_src_len, embed_dim
        padding_idx = self.embeddings.padding_idx
        tgt_pad_mask = tgt_words.data.eq(padding_idx).unsqueeze(1) \
            .expand(tgt_batch, tgt_len, tgt_len).bool()

        if (not memory_masks is None):
            src_len = memory_masks.size(-1)      # memory_masks: real_batch_size x 1 x max_src_len
            src_pad_mask = memory_masks.expand(src_batch, tgt_len, src_len).bool()

        else:
            src_pad_mask = src_words.data.eq(padding_idx).unsqueeze(1) \
                .expand(src_batch, tgt_len, src_len).bool()
