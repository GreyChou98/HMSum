...
class GraphBatch(object):
    def __init__(self, data=None, hier=False, pad_id=None, device=None, is_test=False):
        if data is not None:
            ...
            self.max_cluster_token = max([max([len(c) for c in cs]) for cs in clusters])
            _src = [self._pad_para(ex, self.max_npara, self.max_ntoken, self.pad_id) for ex in src]
            ...
            #             c2p[str(i)] = {}
            #         for j in range(len(ex['clusters'])):
            #             Ei.append(c2p[str(i)])
            #         Eij.append(Ei)
            #     batch_E.append(Eij)
            # batch_E = torch.tensor(batch_E).to(device)
            #
            ...
            _tgt = self._pad_para(tgt, width=max([len(d) for d in tgt]), height=len(tgt), pad_id=pad_id)
    ...
    def _pad_cluster(self, clusters, pad_id, _max_cluster_token = 50):
        ...
        if _max_cluster_token > self.max_cluster_token:
            _max_cluster_token = self.max_cluster_token
    ...
    def _pad_para(self, data, height, width, pad_id):
        """
