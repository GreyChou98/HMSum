...
class GraphBatch(object):
    def __init__(self, data=None, hier=False, pad_id=None, device=None, is_test=False):
        if data is not None:
            ...
            self.max_cluster_ntoken = max([max([len(c) for c in cs]) for cs in clusters])
            ...
            _cluster = [self._pad_para(ex, self.max_ncluster, self.max_cluster_ntoken, self.pad_id) for ex in clusters]
